...
- An `api` directory, containing a single endpoint that retrieves the current time with the JSON API built with the [@now/node Builder](https://zeit.co/docs/v2/deployments/official-builders/node-js-now-node/).
- A `www` directory, containing a single static file, `index.html`, that shows a frontend with information from the API.

## Get Started with This Project

To get started with this project yourself, you can use [Now CLI](https://zeit.co/download) to initialize it.

From your terminal, use the following command to create a directory called `my-json-api-project` including the files of this example:

```bash
now init json-api my-json-api-project
```

Then, `cd` into your new project's directory (with `cd my-json-api-project`).

Lastly, add your JSON data to the `db.js` file, using only arrays and objects, to create your API in seconds.
